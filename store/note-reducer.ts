...
export function noteReducer(state: IAppState, action: IAction): IAppState {
  switch (action.type) {
    ...
    case ActionType.SET_ACTIVE_NOTE: {
      ...
      const newActiveNote = notes?.find((note) => note.nanoid === action.note?.nanoid);
      if (newActiveNote) {
    ...
    case ActionType.UPDATE_NOTE_NAME: {
      if (!state.activeNote) throw Error("noteReducer called: Note is not active: " + action.type);
      // HACK: Chances of error here. if user updates docs aswell as title.
      if (typeof action.note !== 'undefined') {
        return {
          ...state, unsavedData: true, activeNote: { ...state.activeNote, title: action.note?.title, } // This could be action.note.
        };
      } else {
        throw Error("noteReducer called: Undefined action payload: " + action.type);
      }
    }
    case ActionType.UPDATE_MARKDOWN: {
      if (!state.activeNote) throw Error("noteReducer called: Note is not active: " + action.type);
      if (typeof action.note === "undefined") throw Error("noteReducer called: Undefined action payload: " + action.type);
      return {
        ...state,
    ...
    case ActionType.ADD_NOTE: {
      if (typeof action.note !== 'undefined') {
        ...
        return { ...state, unsavedData: false, isPreview: false, activeNote: action.note, notes: state.notes }
      }
    }
    case ActionType.DELETE_NOTE: {
      if (!state.notes) throw Error("noteReducer called: No notes left to delete: " + action.type);
      const newNotes = state.notes.filter((note) => note.nanoid !== action.note?.nanoid);
